dashboard "alicloud_kms_key_age_report" {
  ...
  title         = "Alibaba Cloud Key Age Report"
...
query "alicloud_kms_key_24_hours_count" {
  sql = <<-EOQ
    select
      count(*) as value,
      '< 24 hours' as label
...
query "alicloud_kms_key_30_days_count" {
  sql = <<-EOQ
     select
      count(*) as value,
      '1-30 Days' as label
...
query "alicloud_kms_key_30_90_days_count" {
  sql = <<-EOQ
     select
      count(*) as value,
      '30-90 Days' as label
...
query "alicloud_kms_key_90_365_days_count" {
  sql = <<-EOQ
    select
      count(*) as value,
...
query "alicloud_kms_key_1_year_count" {
  sql = <<-EOQ
    select
      count(*) as value,
      '> 1 Year' as label
...
query "alicloud_kms_key_age_table" {
  sql = <<-EOQ
    select
      k.key_id as "Key ID",
