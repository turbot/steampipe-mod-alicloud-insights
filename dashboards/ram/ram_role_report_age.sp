dashboard "ram_role_age_report" {
  ...
  container {
    ...
    card {
      query = query.ram_role_24_hours_count
      width = 2
      type  = "info"
    }

    card {
      query = query.ram_role_30_days_count
      width = 2
      type  = "info"
    }

    card {
      query = query.ram_role_30_90_days_count
      width = 2
      type  = "info"
    }

    card {
      query = query.ram_role_90_365_days_count
      width = 2
      type  = "info"
    }

    card {
      query = query.ram_role_1_year_count
      width = 2
      type  = "info"
    }
  ...
  table {
    ...
    column "Name" {
      href = "${dashboard.ram_role_detail.url_path}?input.role_arn={{.ARN | @uri}}"
    }

    query = query.ram_role_age_table
  }

}

query "ram_role_24_hours_count" {
  sql = <<-EOQ
    select
      count(*) as value,
      '< 24 hours' as label
...
query "ram_role_30_90_days_count" {
  sql = <<-EOQ
    select
      count(*) as value,
      '30-90 Days' as label
...
query "ram_role_90_365_days_count" {
  sql = <<-EOQ
    select
      count(*) as value,
