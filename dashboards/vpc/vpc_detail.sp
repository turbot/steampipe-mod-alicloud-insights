dashboard "alicloud_vpc_detail" {
  ...
  container {
    ...
    card {
      width = 2
      query = query.alicloud_vpc_num_ips_for_vpc
      args = {
    ...
    card {
      width = 2
      query = query.aws_vswitch_count_for_vpc
      args = {
        vpc_id = self.input.vpc_id.value
      }
    }
  ...
  container {
    ...
    chart {
      ...
      type  = "column"
      width = 4
      query = query.alicloud_vpc_subnet_by_az
      args = {
        vpc_id = self.input.vpc_id.value
      }
    ...
    table {
      query = query.alicloud_vpc_subnets_for_vpc
      width = 6
      args = {
        vpc_id = self.input.vpc_id.value
      }
  ...
  container {
    ...
    flow {
      query = query.alicloud_vpc_routes_for_vpc_sankey
      args = {
        vpc_id = self.input.vpc_id.value
      }
    }
    ...
    table {
      title = "Route Tables"
      query = query.alicloud_vpc_route_tables_for_vpc
      width = 6
      args = {
        vpc_id = self.input.vpc_id.value
      }
    ...
    table {
      title = "Routes"
      query = query.alicloud_vpc_routes_for_vpc
      width = 6
      args = {
        vpc_id = self.input.vpc_id.value
      }
  ...
  container {
    ...
    flow {
      title = "Peering Connections"
      width = 6
      query = query.alicloud_vpc_peers_for_vpc_sankey
      args = {
        vpc_id = self.input.vpc_id.value
      }
    ...
    table {
      title = "Peering Connections"
      query = query.alicloud_vpc_peers_for_vpc
      args = {
        vpc_id = self.input.vpc_id.value
      }
    }
  ...
  container {
    ...
    table {
      ...
      query = query.alicloud_vpc_endpoints_for_vpc
      width = 6
      args = {
        vpc_id = self.input.vpc_id.value
      }
    ...
    table {
      title = "Gateways"
      query = query.alicloud_vpc_gateways_for_vpc
      width = 6
      args = {
        vpc_id = self.input.vpc_id.value
      }
  ...
  container {
    ...
    table {
      query = query.alicloud_vpc_security_groups_for_vpc
      width = 12
      args = {
        vpc_id = self.input.vpc_id.value
      }
      ...
      column "Group Name" {
        href = "${dashboard.alicloud_vpc_security_group_detail.url_path}?input.security_group_id={{.'Group ID' | @uri}}"
      }
    }
...
query "alicloud_vpc_num_ips_for_vpc" {
  sql = <<-EOQ
    with cidrs as (
      select
...
query "alicloud_vpc_subnets_for_vpc" {
  sql = <<-EOQ
    with subnets as (
      select
        subnet_id,
...
query "alicloud_vpc_security_groups_for_vpc" {
  sql = <<-EOQ
    select
...
query "alicloud_vpc_endpoints_for_vpc" {
  sql = <<-EOQ
    select
...
query "alicloud_vpc_route_tables_for_vpc" {
  sql = <<-EOQ
    select
...
query "alicloud_vpc_routes_for_vpc" {
  sql = <<-EOQ
    select
      route_table_id as "Route Table ID",
      tags ->> 'Name' as "Name",
...
query "alicloud_vpc_peers_for_vpc" {
  sql = <<-EOQ
    select
      id as "ID",
      tags ->> 'Name' as "Name",
    ...
    from
      alicloud_vpc_peering_connection
    where
      requester_vpc_id = $1
...
query "alicloud_vpc_gateways_for_vpc" {
  sql = <<-EOQ
    select
      internet_gateway_id as "ID",
      tags ->> 'Name' as "Name",
      'alicloud_vpc_internet_gateway' as "Type",
      a ->> 'State' as "State"
    from
      alicloud_vpc_internet_gateway,
    ...
    select
      id as "ID",
      tags ->> 'Name' as "Name",
      'alicloud_vpc_egress_only_internet_gateway' as "Type",
      a ->> 'State' as "State"
    from
      alicloud_vpc_egress_only_internet_gateway,
      jsonb_array_elements(attachments) as a
     where
      a ->> 'VpcId' = reverse(split_part(reverse($1), '/', 1))
    union all
...
query "alicloud_vpc_peers_for_vpc_sankey" {
  sql = <<-EOQ
    with peers as (
      select
        id,
...
query "alicloud_vpc_subnet_by_az" {
  sql = <<-EOQ
    select
      zone_id,
      count(*)
