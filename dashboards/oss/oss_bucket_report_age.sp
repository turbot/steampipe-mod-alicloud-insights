dashboard "alicloud_oss_bucket_age_report" {
  ...
  container {
    ...
    card {
      query = query.alicloud_oss_bucket_24_hours_count
      width = 2
      type  = "info"
    }

    card {
      query = query.alicloud_oss_bucket_30_days_count
      width = 2
      type  = "info"
    }

    card {
      query = query.alicloud_oss_bucket_30_90_days_count
      width = 2
      type  = "info"
    }

    card {
      query = query.alicloud_oss_bucket_90_365_days_count
      width = 2
      type  = "info"
    }

    card {
      query = query.alicloud_oss_bucket_1_year_count
      width = 2
      type  = "info"
    }
  ...
  table {
    ...
    query = query.alicloud_oss_bucket_age_table
  }

}

query "alicloud_oss_bucket_24_hours_count" {
  sql = <<-EOQ
    select
      count(*) as value,
      '< 24 hours' as label
...
query "alicloud_oss_bucket_30_days_count" {
  sql = <<-EOQ
    select
      count(*) as value,
      '1-30 Days' as label
...
query "alicloud_oss_bucket_30_90_days_count" {
  sql = <<-EOQ
    select
      count(*) as value,
      '30-90 Days' as label
...
query "alicloud_oss_bucket_90_365_days_count" {
  sql = <<-EOQ
    select
      count(*) as value,
...
query "alicloud_oss_bucket_1_year_count" {
  sql = <<-EOQ
    select
      count(*) as value,
      '> 1 Year' as label
...
query "alicloud_oss_bucket_age_table" {
  sql = <<-EOQ
    select
      b.name as "Name",
