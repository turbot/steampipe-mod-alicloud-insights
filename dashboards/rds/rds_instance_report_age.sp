dashboard "rds_instance_age_report" {
  ...
  container {
    ...
    card {
      type  = "info"
      width = 2
      query = query.rds_instance_24_hours_count
    }

    card {
      type  = "info"
      width = 2
      query = query.rds_instance_30_days_count
    }

    card {
      type  = "info"
      width = 2
      query = query.rds_instance_30_90_days_count
    }

    card {
      width = 2
      type  = "info"
      query = query.rds_instance_90_365_days_count
    }

    card {
      width = 2
      type  = "info"
      query = query.rds_instance_1_year_count
    }
  ...
  table {
    ...
    query = query.rds_instance_age_table
  }

}

query "rds_instance_24_hours_count" {
  sql = <<-EOQ
    select
      count(*) as value,
      '< 24 hours' as label
...
query "rds_instance_30_days_count" {
  sql = <<-EOQ
    select
      count(*) as value,
      '1-30 Days' as label
...
query "rds_instance_30_90_days_count" {
  sql = <<-EOQ
    select
      count(*) as value,
      '30-90 Days' as label
...
query "rds_instance_90_365_days_count" {
  sql = <<-EOQ
    select
      count(*) as value,
...
query "rds_instance_1_year_count" {
  sql = <<-EOQ
    select
      count(*) as value,
      '> 1 Year' as label
